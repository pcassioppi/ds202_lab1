...
You may work on this with your partner or individually. 
  ...
  Git is a version control system that helps users by tracking changes in codes and projects. Git can bu used with Bash to access and manage repositories and documents in a shell, similar to windows command line. Git is great for saving and working with code, as the repository tracks any changes made to a project. Github, however, is a site that allows users to save repositories on, so that other users can access them and see them.
  
  2. Can Git and Github be used independently? Why don't we do so? Describe the advantages and disadvantages. 
  
  Git and Github **can** be used independently. The reason we use them together is because they are made for each other, and, as a result, it is very easy to work with Github using Git. One great thing about Github, is that you can read and access version histories of projects very easily. A disadvantage may be that there is a learning curve when it comes to using Git, as there are commands that some users won't know if they don't have experience with command line prompts.

  3. What is a repository in Git and how can you create a repository in Git (write out the command(s))? 

  A repository in Git is a type of file that saves projects, while also saving all of their previous versions.

  To create a repository in Git you would follow these steps in the terminal:
  
    1. Make a directory after you are where you want your repository - $ mkdir Lab1Repo
  
    2. Change your directory so you are in the directory you want to make your repository - $ cd Lab1Repo
  
    3. Use git init to initialize an empty repository - $ git init
    
    Now you have your repository!
...
# Coding Exercises 
  ...
  `data.frame`. The advantage of `data.frame` is that it can handle vectors of different types. For example, age is a numeric 
    ...
    ```
    example_data$weight[example_data$weight == 0] = 150
    ```
  ...
  ```{r, eval=FALSE}
  null_mean = function(x){
  mean = mean(is.na(x))
  return(mean)
  }

  vector_share = function(x){
  percent = x / sum(x, na.rm = TRUE)
  return(percent)
  }
 
  ```
 These both only need one argument (a vector)
...
4. Write a function named `both_na()`. This function take two vectors of the same length and returns the number of positions that have an `NA` value in both vectors.
  ...
  total_na = sum(sum(is.na(x))+sum(is.na(y)))
  return(total_na)
  }
